...
Corners: the side wall yields to the back/front run (v1 rule); the user manages any corner
...
`migrateProject`, which adds one to every `shelves` zone `count`, reproducing exactly the
geometry it was saved with. Autosaved v1 projects take the same path. Defaults from §2.7 when nothing is stored.
